package com.enonic.xp.app.users.handler;

import org.junit.jupiter.api.Test;

import static org.junit.jupiter.api.Assertions.assertEquals;

public class KidGeneratorHandlerTest
{
    @Test
    public void testGenerateKid()
    {
        String kid = "f970ced1673f0a2790b526ad03625e38";
        String key =
            "-----BEGIN PUBLIC KEY-----\\n-----END PUBLIC KEY-----\n";
        KidGeneratorHandler generator = new KidGeneratorHandler();
        assertEquals( kid, generator.generateKid( key ) );
    }
}
